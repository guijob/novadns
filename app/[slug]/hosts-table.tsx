...
export function HostsTable({ slug, hosts: initialHosts, base, plan, isTeam, groups, userTeams }: {
  slug: string
  hosts: Host[]
  base: string
  plan: string
  ...
  useEffect(() => {
    const es = new EventSource(`/api/hosts/stream?slug=${encodeURIComponent(slug)}`)
    es.onmessage = (e) => {
  ...
  const planLimit   = (isTeam && !isPaidPlan(plan)) ? 0 : getPlanLimit(plan)
  const atLimit     = activeCount >= planLimit
  const nearLimit   = !atLimit && planLimit > 0 && activeCount / planLimit >= 0.8
  ...
  async function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await addHost(slug, new FormData(e.currentTarget))
  ...
  return (
    <>
      {/* ── Limit banner ─────────────────────────────────────────── */}
      {nearLimit && (
        <div className="flex items-center gap-3 border border-yellow-300 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/40 px-4 py-2.5">
          <HugeiconsIcon icon={Alert02Icon} strokeWidth={1.5} className="size-4 text-yellow-600 dark:text-yellow-500 shrink-0" />
          <p className="text-sm text-yellow-800 dark:text-yellow-300">
            You&apos;re using <strong>{activeCount} of {planLimit}</strong> hosts on your {PLANS[plan as keyof typeof PLANS]?.label ?? plan} plan.{" "}
            {!isPaidPlan(plan)
              ? <a href={`/${slug}/settings`} className="underline underline-offset-2 font-medium">Upgrade</a>
              : <a href={`/${slug}/settings`} className="underline underline-offset-2 font-medium">Upgrade your plan</a>
            }{" "}to get more.
          </p>
        </div>
      )}
            ...
            <>
              ...
              <div className="px-6 flex flex-col items-center text-center gap-4 py-8">
                ...
                <div className="space-y-1">
                  {isTeam && !isPaidPlan(plan) ? (
                    <>
                      <p className="font-semibold">Subscription required</p>
                      <p className="text-sm text-muted-foreground">
                        This team needs an active plan to add hosts.{" "}
                        <a href={`/${slug}/settings`} className="underline underline-offset-2">Subscribe</a> from the team settings.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-sm text-muted-foreground">
                        You&apos;ve used all {planLimit} active hosts on your plan.
                        {!isPaidPlan(plan) && <> <a href={`/${slug}/settings`} className="underline underline-offset-2">Upgrade</a> to add more.</>}
                      </p>
                    </>
                  )}
                </div>
